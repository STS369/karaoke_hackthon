...
import {
  fetchMe,
  fetchRecentlyPlayed,
  fetchRecommendationsFromRecent,
  fetchLikedTracks,
  loginWithSpotify,
} from "../api/spotify"
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.error(e)
        setLikedError(
          "お気に入りの取得に失敗しました。スコープ(user-library-read)が不足している可能性があります。ヘッダーからログアウトし、再ログインしてください。",
        )
      } finally {
    ...
    <div className="page">
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem" }}>
        <section style={{ flex: 1 }}>
          <h2 style={{ fontSize: "1.25rem" }}>お気に入りの曲</h2>
          {likedLoading ? (
            <div>読み込み中…</div>
          ) : likedError ? (
        ...
        <section style={{ flex: 1 }}>
          <h2 style={{ fontSize: "1.25rem" }}>最近聴いた曲</h2>
          <RecentPlayList tracks={recent} onAddSelected={addSelected} />
        </section>
      </div>
      <section>
        <h2 style={{ fontSize: "1.25rem" }}>おすすめ曲</h2>
        {recsLoading ? (
          <div>おすすめを準備中…</div>
        ) : (
